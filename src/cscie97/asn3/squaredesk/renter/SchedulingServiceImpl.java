...
public class SchedulingServiceImpl implements SchedulingService {
	...
	if (booking == null) {
		...
		// OfficeSpace
		if (!checkAvailability(booking.getOfficeSpaceID(),
		    ...
		    // subtracting 1
		    OfficeSpace officeSpace = OfficeSpaceServiceImpl
			    .getInstance().getOffice(AUTHTOKEN,
				    booking.getOfficeSpaceID());
		    int workSpaces = officeSpace.getCapacity().getWorkSpaces();
		    officeSpace.getCapacity().setWorkSpaces(workSpaces--);
		    // update OfficeSpace
		    OfficeSpaceServiceImpl.getInstance().updateOffice(
			    AUTHTOKEN, officeSpace);
	...
	if ((officeSpaceID == null) || (startDate == null) || (endDate == null)) {
	    ...
	    try {
		OfficeSpace officeSpace = OfficeSpaceServiceImpl.getInstance()
			.getOffice(AUTHTOKEN, officeSpaceID);
		...
		if (officeSpace.getCapacity().getWorkSpaces() <= 0)
			...
			while (bookingItr.hasNext()) {
			    ...
			    // verifies if startDate or endDate is between any
			    // of current OfficeSpace Booking list
			    if ((startDate.before(booking.getEndDate()) && startDate
				    .after(booking.getStartDate()))
				    || (endDate.before(booking.getEndDate()) && endDate
					    .after(booking.getStartDate()))
				    || (startDate
					    .equals(booking.getStartDate()))
				    || (startDate.equals(booking.getEndDate()))
				    || (endDate.equals(booking.getStartDate()) || (endDate
					    .equals(booking.getEndDate())))) {
				return false;
			    } else {
				return true;
			    }
	...
	if (bookingID == null) {
	    ...
	    BookingNotFoundException ex = new BookingNotFoundException();
	    ex.setDescription("Booking ID: '" + bookingID
		    + "' does not exists in our records.");
	    throw ex;
	} else if (bookingMap.containsKey(bookingID)) {
	    bookingMap.remove(bookingMap.get(bookingID));
	    System.out.println("Deletion of Booking ID: '" + bookingID
		    + "' completed.\n");
	}
    }
